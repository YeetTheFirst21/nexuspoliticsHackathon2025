...
def main():
    ...
    with col1:
        ...
        if len(selectedCities) > 0:
            option = st.selectbox("🧳Select Starting City", districts_data["category"].tolist(), index=cities["Capital"].tolist().index(selectedCities[0]["Capital"]))
        else:
            option = st.selectbox("🧳Select Starting City", districts_data["category"].tolist(), index=0)
    with col2:
        if len(selectedCities) > 1:
            option2 = st.selectbox("🏠Select Destination City", districts_data["category"].tolist(), index=cities["Capital"].tolist().index(selectedCities[1]["Capital"]))
        else:
            option2 = st.selectbox("🏠Select Destination City", districts_data["category"].tolist(), index=1)
